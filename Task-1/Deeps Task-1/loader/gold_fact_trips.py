...
def fact_trip():
    ...
    return (
        ...
        # Inner join with dim_datetime — only trips with valid date_key
        .join(dates, on="date_key", how="inner")

        # Inner join with dim_location (pickup) — only trips with valid pickup zone
        .join(pickup, on="pickup_location_id", how="inner")

        # Inner join with dim_location (dropoff) — only trips with valid dropoff zone
        .join(dropoff, on="dropoff_location_id", how="inner")
